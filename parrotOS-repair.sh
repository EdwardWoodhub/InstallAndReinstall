...
# 验证关键文件
if ! find "$MOUNT_SYS/boot" -name "vmlinuz*" -print -quit | grep -q .; then
  echo "错误：内核文件缺失！"
  exit 1
fi
...
mount --bind /run  "$MOUNT_SYS/run"
mkdir -p "$MOUNT_SYS/dev/pts"
mount --bind /dev/pts "$MOUNT_SYS/dev/pts"

# 复制 DNS 配置
cp /etc/resolv.conf "$MOUNT_SYS/etc/resolv.conf"
...
chroot "$MOUNT_SYS" /bin/bash -c " \
  ...
  passwd -l root"  # 禁用 root 密码


# 安装 GRUB 到 MBR（关键修改）
echo "安装 GRUB...;生成 GRUB 配置..."
grub-install \
  --target=i386-pc \
  --root-directory="$MOUNT_SYS" \
  --recheck \
  --force \
  "$TARGET_DISK"
...
chroot "$MOUNT_SYS" /bin/bash -c " \
  apt update && \
  apt install -y grub-common grub2-common grub-pc-bin && \
  update-grub"
