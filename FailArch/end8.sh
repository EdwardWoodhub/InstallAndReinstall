...
ROOT_PASSWORD="endroot2025"          # 预设root密码
# ================================================
...
download_and_install_package() {
    ...
    if [ -z "$url" ]; then
        echo "错误：无法获取 $package 的下载链接"
        return 1
    fi

    local tmpdir="/tmp/pacman_download"
    mkdir -p "$tmpdir"
    
    echo "尝试直接下载 $package..."
    if ! curl -L -o "$tmpdir/$package.pkg.tar.zst" "$url"; then
        echo "错误：下载 $package 失败"
        return 1
    fi

    echo "尝试安装 $package..."
    if ! pacman -U --nocheck "$tmpdir/$package.pkg.tar.zst" --noconfirm; then
...
for pkg in "${packages[@]}"; do
    for i in {1..3}; do
        ...
        echo "镜像源尝试失败，尝试直接下载安装 $pkg..."
        if ! download_and_install_package "$pkg"; then
            echo "致命错误：无法安装 $pkg"
            exit 1
        fi
    }
...
echo ">> 设置root密码..."
ENCRYPTED_PW=$(openssl passwd -6 -salt xyz endroot2025 2>/dev/null)
if [ -z "$ENCRYPTED_PW" ]; then
    echo "密码加密失败！请检查openssl是否安装"
    exit 1
fi

if ! sed -i "s#^root:[^:]*#root:${ENCRYPTED_PW}:#" /etc/shadow; then
    echo "密码写入失败！可能原因："
    echo "1. /etc/shadow不可写"
    echo "2. sed命令执行失败"
    exit 1
fi

if ! grep -q "root:${ENCRYPTED_PW}" /etc/shadow; then
    echo "密码验证失败！"
    exit 1
fi
echo "root密码已成功设置为endroot2025"
