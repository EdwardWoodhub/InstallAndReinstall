...
echo ">> 更新软件数据库 (强制模式)..."
pacman -Syy --noconfirm --overwrite '*'

echo ">> 重新安装核心组件..."
pacman -S --noconfirm --overwrite '*' \
  systemd glibc linux linux-headers linux-firmware \
  mkinitcpio grub
...
echo ">> 生成initramfs..."
cat > /etc/mkinitcpio.conf <<INIT_CONF
MODULES=()
BINARIES=()
FILES=()
...
lsinitcpio /boot/initramfs-linux.img | grep -E 'systemd|fsck'
EOF

    # 二次验证
    echo "=== 安装后验证 ==="
    if [ ! -x "$MOUNT_DIR/usr/lib/systemd/systemd" ]; then
        echo "错误：systemd二进制文件缺失或不可执行"
        ls -l "$MOUNT_DIR/usr/lib/systemd/systemd"
        exit 1
    fi
    
    if [ ! -f "$MOUNT_DIR/boot/initramfs-linux.img" ]; then
        echo "错误：initramfs未正确生成"
        exit 1
    fi

    echo "=== GRUB配置验证 ==="
    grep 'init=/usr/lib/systemd/systemd' "$MOUNT_DIR/boot/grub/grub.cfg" || {
        echo "错误：GRUB缺少systemd初始化参数"
        exit 1
    }
}
